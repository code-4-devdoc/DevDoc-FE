  ...
  const fetchResumes = useCallback(async () => {
    try {
      ...
      const formattedResumes = data.map(resume => ({
        id: resume.id,
        title: resume.title,
        createdAt: resume.createdAt  // createdAt 필드 추가
      }));
  ...
  const createResume = useCallback(async () => {
    try {
      const response = await fetch(`${baseUrl}/api/resumes`, {
        ...
        body: JSON.stringify({ title: newTitle }), // Title을 JSON 형식으로 전송
      });
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
      ...
      <ul style={{ listStyle: 'none', padding: 0 }}>
        {resumes.map(resume => (
          <li key={resume.id} style={{ marginTop: '1rem' }}>
            <button onClick={() => navigate(`/resumes/${resume.id}`)}>{`이력서 ${resume.id}. ${resume.title} (Created at: ${new Date(resume.createdAt).toLocaleString()})`}</button>
